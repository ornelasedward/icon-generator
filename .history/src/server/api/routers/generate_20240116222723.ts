...
async function generateIcon(prompt: string): Promise<string | undefined> {
    if (env.MOCK_DALLE === 'true') {
        ...
        const response = await openai.images.generate({
            prompt: prompt, // Changed from input.prompt to prompt
        });

        // Assuming the response.data is an array of images, return the first image's URL.
        // Adjust this according to the actual structure of response.data.
        return response.data[0].url; // Example: Adjust this line based on your response structure
    }
}
